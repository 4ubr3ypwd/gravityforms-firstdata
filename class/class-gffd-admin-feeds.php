...
class GFFD_Admin_Feeds {
	...
	function __construct( $gffd_core ) {
		...
		add_filter( 'gform_addon_navigation', array( $this, 'gffd_admin_feeds_add_navigation' ) );
		add_action( 'admin_init', array( $this, 'gffd_admin_feeds_save_feed' ) );
	}
	...
	function gffd_feed_admin_url( $href_echo = null, $echo = null ) {
		$url = 'admin.php?page=' . $this->gffd_admin_feeds_add_navigation_config( 'name' );
	...
	function gffd_admin_feeds_page(){
		...
		if ( gffd_admin_feeds_is_subpage( 'add-new-edit' ) ) {
			...
			require 'gffd-gfadmin-feeds.html.php';
		}
	}
